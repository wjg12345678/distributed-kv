...
这是一个 C++17 实现的分布式 KV 存储系统，核心是 Raft 共识。客户端写请求进入 leader 后会被包装成日志，通过 AppendEntries 复制到 follower，多数派确认后推进 commit index，再按顺序应用到 KV 状态机。项目还实现了 Pre-Vote、线性一致读、snapshot、InstallSnapshot、RocksDB 持久化、基于 libuv TCP 长连接 + Protobuf 的多进程通信、基于 libuv + llhttp 的 HTTP API、锁服务、MVCC 和成员变更接口。
...
4. 工程联调：多进程 libuv TCP RPC、llhttp HTTP API、RocksDB、测试脚本要组合起来跑通。
...
- 网络 RPC 层：libuv TCP 长连接 + Protobuf 传输 Raft RPC。
- 状态机层：KV、锁、MVCC、成员配置。
...
### 61. 为什么要做 libuv TCP 长连接 + Protobuf？

单进程内存调用只能验证算法，不能展示真实部署中的序列化、网络连接、独立进程和端口。libuv TCP 长连接让节点间 RPC 具备真实网络 I/O、连接复用和异步事件循环；Protobuf 负责结构化序列化。两者组合比直接函数调用更接近真实分布式系统。
...
通过单元测试覆盖选举、复制、异步复制、冲突覆盖、PreVote、线性一致读、快照、持久化、锁、MVCC、成员变更和 TCP 超时。再通过本地多进程脚本验证真实 libuv TCP 节点的启动、leader 查询、KV 写读和接口路径。
...
最能体现的是我能把 Raft 论文里的选举、日志复制、提交、安全读、快照和持久化落到 C++ 工程代码里，并通过 libuv 多进程 TCP、llhttp HTTP API、RocksDB、测试和脚本形成可运行项目。同时我知道它和生产级系统的差距，不会过度包装。
...
设计基于 libuv TCP 长连接 + Protobuf 的多进程 Raft RPC 通信和基于 libuv + llhttp 的 HTTP 接入层，支持 follower leader redirect、KV 读写、锁服务、MVCC 读写、成员变更接口、状态查询和 metrics 暴露，并提供本地 3 节点启动与验证脚本。
